(function () {
    ...
    var scrollFrame = function (selector) {
        ...
        document.addEventListener('click', function (e) {
            ...
            if (!elMatchesSelector)
                return;

            // Get the href & open the iframe on that url
            var href = e.target.parentNode.parentNode.href || e.target.parentNode.href || e.target.href;
            if (href) {
                e.preventDefault();
                openIframe(href);
            }
    ...
    var openIframe = function (url) {
        ...
        var onPopState = function (e) {
            ...
            var logoHistory = window.localStorage.getItem('logoListHistory');
            loadLogoHistory(logoHistory);
    ...
    var refreshOnNewIframePage = function () {
        addEventListener('message', function (e) {
            if (!e.data.href)
                return;
            if (!e.data.scrollFrame == true)
                return;
            if (e.data.href == this.location.href)
                return;
            var body = document.getElementsByTagName('body')[0];

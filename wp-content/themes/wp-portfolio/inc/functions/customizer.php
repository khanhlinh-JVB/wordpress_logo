...
<?php }
function wp_portfolio_upgrade_notice() {
	// Enqueue the script
	wp_enqueue_script(
		'wp_portfolio-upgrade-pro',
		get_template_directory_uri() . '/js/wp_portfolio_customizer.js',
		array(), '1.0',
		true
	);
	// Localize the script
	wp_localize_script(
		'wp_portfolio-upgrade-pro',
		'wp_portfolioproupgrade',
		array(
			'wp_portfolioprourl'		=> esc_url( 'http://themehorse.com/themes/wp-portfolio-pro/' ),
			'wp_portfolioprolabel'	=> __( 'Upgrade to WP Portfolio Pro', 'wp-portfolio' ),
		)
	);
}
add_action( 'customize_controls_enqueue_scripts', 'wp_portfolio_upgrade_notice' );
